...
const Badge = ({label, color = 'primary.main'}) => (
    <Box sx={{
        ...
        <Typography sx={{fontFamily: FONT, fontSize: '10px', fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.4px', lineHeight: 1}}>
            {label}
        </Typography>
    </Box>
);
        ...
        <Box sx={{width: '100%', mt: '40px', display: 'flex', flexDirection: 'column', gap: '14px'}}>
            ...
            <Box sx={{
                ...
                <Box sx={{flex: 1, p: {xs: '36px 28px', md: '52px 48px'}, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '16px'}}>
                    ...
                    <Typography sx={{fontFamily: FONT, fontSize: {xs: '28px', md: '36px'}, fontWeight: 900, color: 'text.primary', lineHeight: 1.05, letterSpacing: '-1px', textTransform: 'uppercase'}}>
                        Raízes e sabores:<br/>afeto em cada parte<br/>do alimento
                    </Typography>
                    <Typography sx={{fontFamily: FONT, fontSize: '13px', color: 'text.secondary', lineHeight: '22px', maxWidth: '420px'}}>
                ...
                {PILLARS.map((p) => {
                    ...
                    return (
                        <Box key={p.title} sx={{
                            ...
                            <Typography sx={{fontFamily: FONT, fontSize: '14px', fontWeight: 700, color: 'text.primary', lineHeight: 1.3, letterSpacing: '-0.2px'}}>
                                {p.title}
                            </Typography>
                            <Typography sx={{fontFamily: FONT, fontSize: '12px', color: 'text.secondary', lineHeight: '18px'}}>
                                {p.text}
                            </Typography>
                        </Box>
                    );
            ...
            <Box sx={{display: 'flex', flexDirection: {xs: 'column', md: 'row'}, gap: '14px'}}>
                ...
                <Box sx={{
                    ...
                    <Box sx={{p: {xs: '28px', md: '36px 36px 24px'}, display: 'flex', flexDirection: 'column', gap: '12px', flex: 1}}>
                        ...
                        <Typography sx={{fontFamily: FONT, fontSize: {xs: '18px', md: '22px'}, fontWeight: 800, color: 'text.primary', letterSpacing: '-0.5px', lineHeight: 1.2}}>
                            Sabores que contam histórias
                        </Typography>
                        <Typography sx={{fontFamily: FONT, fontSize: '13px', color: 'text.secondary', lineHeight: '22px'}}>
                ...
                <Box sx={{
                    ...
                    <Box sx={{backgroundColor: 'sustainable.main', px: '28px', py: '24px'}}>
                        ...
                        <Typography sx={{fontFamily: FONT, fontSize: '20px', fontWeight: 900, color: '#fff', letterSpacing: '-0.5px', lineHeight: 1.2, mt: '10px', textTransform: 'uppercase'}}>
                            Projeto de<br/>Mestrado
                        </Typography>
                        <Typography sx={{fontFamily: FONT, fontSize: '12px', color: 'rgba(255,255,255,0.75)', lineHeight: '18px', mt: '8px'}}>
                    ...
                    <Box sx={{px: '24px', py: '20px', display: 'flex', flexDirection: 'column', gap: '0px', flex: 1}}>
                        <Typography sx={{fontFamily: FONT, fontSize: '10px', fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '0.5px', mb: '14px'}}>
                            Equipe
                        </Typography>
                        {TEAM_GROUPS.map((group, i) => (
                            <React.Fragment key={group.role}>
                                <Box sx={{py: '10px'}}>
                                    <Typography sx={{fontFamily: FONT, fontSize: '10px', fontWeight: 700, color: 'text.primary', textTransform: 'uppercase', letterSpacing: '0.4px', mb: '6px'}}>
                                        {group.role}
                                    </Typography>
                                    <Box sx={{display: 'flex', flexDirection: 'column', gap: '2px'}}>
                                        {group.names.map((name) => (
                                            <Typography key={name} sx={{fontFamily: FONT, fontSize: '14px', fontWeight: 400, color: 'text.primary', letterSpacing: '-0.2px', lineHeight: 1.4}}>
                                                {name}
                                            </Typography>
                                        ))}
                                    </Box>
